...
export async function findPool(
  ...
  try {
    ...
    if (dexLower === "uniswapv4") {
            const factory = getContract(DEX_FACTORY.uniswapv4Factory, FACTORY_ABIS.UNISWAP_V3);
      const pool = await factory.getPool(token0, token1, fee);
      return pool;
    }
    ...
    if (dexLower === "sushiswapv3") {
            const factory = getContract(DEX_FACTORY.aerodromeFactory, FACTORY_ABIS.UNISWAP_V3);
      const pool = await factory.getPool(token0, token1, fee);
      return pool;
    }
    ...
    if (dexLower === "aerodromeslipstream") {
      const factory = getContract(DEX_FACTORY.aerodromeslipstreamFactory, FACTORY_ABIS.UNISWAP_V3);
      const pool = await factory.getPool(token0, token1, fee);
      return pool;
    }
      if (dexLower === "alienbasev3") {
    ...
    if (dexLower === "baseswapv3") {
      const factory = getContract(DEX_FACTORY.aerodromeFactory, FACTORY_ABIS.UNISWAP_V2);
