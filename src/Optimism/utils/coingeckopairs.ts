...
const COINGECKO_ARBITRUM_URL = 'https://tokens.coingecko.com/optimism-one/all.json';
const OUTPUT_FILE = path.join(__dirname, 'optimism_tokens.json');
...
export async function fetchTopTokensOptimism(
  limit: number = 200
): Promise<TokenInfo[]> {
  const now = Date.now();

  if (cache && now - cache.timestamp < CACHE_DURATION_MS) {
    console.log('✅ Usando cache de tokens Optimism.');
  ...
  currentFetchPromise = (async () => {
    try {
      ...
      console.log('✅ Tokens da Optimism atualizados e cacheados.');
      ...
      console.warn('⚠️ Usando fallback local com tokens mais comuns da Optimism.');
