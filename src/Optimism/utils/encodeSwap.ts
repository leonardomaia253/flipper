...
import {uniswapv3Router,solidlyv3Router,velodromeslipstreamRouter,velodromefinancev2Router} from "../constants/addresses";
import { EXECUTOR_CONTRACTARBITRUM, EXECUTOR_CONTRACTOPTIMISM } from "../constants/contracts";
import * as dotenv from "dotenv";
import * as path from "path";
import {multiprovider, PRIVATE_KEY } from '../config/provider';
...
const recipient = EXECUTOR_CONTRACTOPTIMISM



export async function buildSolidlyV3Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(solidlyv3Router, UNISWAP_V2_ROUTER_ABI);
...
export async function buildVelodromefinanceV2Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(velodromefinancev2Router, CAMELOT_ROUTER_ABI);
...
export async function buildVelodromeslipstreamSwap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(velodromeslipstreamRouter, UNISWAP_V3_ROUTER_ABI);
