...
export async function findPool(
  ...
  try {
    ...
    if (dexLower === "velodromeslipstream") {
      const factory = getContract(DEX_FACTORY.velodromeslipstreamFactory, FACTORY_ABIS.UNISWAP_V3);
      const pool = await factory.getPool(token0, token1, fee);
      return pool;
    }

        if (dexLower === "solidlyv3") {
      const factory = getContract(DEX_FACTORY.solidlyv3Factory, FACTORY_ABIS.UNISWAP_V2);
      const pool = await factory.getPair(token0, token1);
      return pool;
    }

    if (dexLower === "velodromefinancev2") {
      const factory = getContract(DEX_FACTORY.velodromefinancev2Factory, FACTORY_ABIS.UNISWAP_V2);
      const pool = await factory.getPair(token0, token1);
      return pool;
    }
