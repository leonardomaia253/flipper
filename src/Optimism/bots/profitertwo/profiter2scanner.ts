...
import { solidlyv3Router, uniswapv3Quoter2, velodromefinancev2Quoter,velodromeslipstreamQuoter } from "../../constants/addresses";
...
const solidlyv3 = new ethers.Contract(solidlyv3Router, IUniswapV2RouterABI, alchemysupport);
const velodromefinancev2 = new ethers.Contract(velodromefinancev2Quoter, IUniswapV2RouterABI, infurasupport);
const uniswapv3 = new ethers.Contract(uniswapv3Quoter2, IUniswapV3QuoterABI, alchemysupport);
const velodromeslipstream = new ethers.Contract(velodromeslipstreamQuoter, IUniswapV3QuoterABI, infurasupport);


type Protocol = "solidlyv3" | "velodromefinancev2" | "velodromeslipstream" | "uniswapv3";
...
async function simulateSwapReal(pool: Pool, amountIn: bigint): Promise<bigint> {
  try {
    if (pool.protocol === "solidlyv3") {
      const amounts: bigint[] = await solidlyv3.getAmountsOut(amountIn, [pool.tokenIn, pool.tokenOut]);
      return amounts[1] ?? 0n;
    }
    if (pool.protocol === "velodromefinancev2") {
      const amounts: bigint[] = await velodromefinancev2.getAmountsOut(amountIn, [pool.tokenIn, pool.tokenOut]);
      return amounts[1] ?? 0n;
    }
    ...
    if (pool.protocol === "velodromeslipstream") {
      const fee = pool.fee ?? 3000;
      const amountOut: bigint = await velodromeslipstream.quoteExactInputSingle(
        pool.tokenIn,
        pool.tokenOut,
        fee,
        amountIn,
...
async function fetchPoolsFromTheGraph(protocol: Protocol, retries = 3, apiKey?: string): Promise<Pool[]> {
  ...
  switch (protocol) {
    case "solidlyv3":
      url = "https://gateway.thegraph.com/api/subgraphs/id/HCThb3gJC45qUYmNEaYmZZTqJW3pSq7X6tb4MqNHEvZf";
      break;
    case "velodromefinancev2":
      url = "https://gateway.thegraph.com/api/subgraphs/id/BsBDqDf6rJJyxKACZrCHAa8Gaf384cmL2hxfLaDuB8XM";
      break;
    case "uniswapv3":
      url = "https://gateway.thegraph.com/api/subgraphs/id/GyZ9MgVQkTWuXGMSd3LXESvpevE8S8aD3uktJh7kbVmc";
      break;
    case "velodromeslipstream":
      url = "https://gateway.thegraph.com/api/subgraphs/id/6jck8kDfV2jYbKqdLZeTSJZK4GvRuPTqNb3yEvNrjHj7";
      break;
  }

  if (["solidlyv3", "velodromefinancev2"].includes(protocol)) {
    query = `
      {
    ...
    const headers: Record<string, string> = { "Content-Type": "application/json" };
       ...
       try {
         ...
         const items = ["solidlyv3", "velodromefinancev2"].includes(protocol) ? data.pairs : data.pools;
    ...
    export async function watcher() {
      ...
      while (true) {
        const allPools = [
          ...(await fetchPoolsFromTheGraph("solidlyv3", 3, apiKey)),
          ...(await fetchPoolsFromTheGraph("velodromefinancev2", 3, apiKey)),
          ...(await fetchPoolsFromTheGraph("velodromeslipstream", 3, apiKey)),
