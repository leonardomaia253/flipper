...
import {uniswapv3Router,lfjv22Router,lfjRouter,lfjv21Router,} from "../constants/addresses";
...
const recipient = EXECUTOR_CONTRACTAVALANCHE


export async function buildLfjSwap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(lfjRouter, UNISWAP_V2_ROUTER_ABI);
...
export async function buildLfjv22Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(lfjv22Router, UNISWAP_V3_ROUTER_ABI);
...
export async function  buildLfjv21Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(lfjv21Router, UNISWAP_V3_ROUTER_ABI);
