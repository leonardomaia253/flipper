...
async function fetchFrom1inch(tokenAddress: string, provider: ethers.Provider): Promise<number | null> {
  try {
    ...
    const url = `https://api.1inch.io/v5.0/43114/quote?fromTokenAddress=${tokenAddress}&toTokenAddress=${WETH_ADDRESS}&amount=${amount.toString()}`;
    const { data } = await axios.get(url, { timeout: TIMEOUT });
