...
export async function fetchTopTokensAvalanche(
  limit: number = 200
): Promise<TokenInfo[]> {
  const now = Date.now();

  if (cache && now - cache.timestamp < CACHE_DURATION_MS) {
    console.log('✅ Usando cache de tokens Avalanche.');
  ...
  currentFetchPromise = (async () => {
    try {
      ...
      console.log('✅ Tokens da Avalanche atualizados e cacheados.');
      ...
      console.warn('⚠️ Usando fallback local com tokens mais comuns da Avalanche.');
