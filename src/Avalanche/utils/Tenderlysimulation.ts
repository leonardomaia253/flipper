...
dotenv.config({ path: path.resolve(__dirname, "../../Avalanche/.env") });
  ...
  try {
    ...
    log.info("Starting Tenderly simulation", {
      ...
      options: {
        ...
        networkId: options.networkId || '43114', // Default to Avalanche
        save: options.save,
      }
    });
    ...
    const simulationPayload = {
      network_id: options.networkId || '43114', // Avalanche
...
export async function estimateBundleProfitability(
  ...
  try {
    ...
    const simulation = await simulateBundleWithTenderly(
      transactions,
      {
        networkId: options.networkId || '43114',
        save: false,
