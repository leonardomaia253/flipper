...
async function fetchFrom1inch(tokenAddress: string, provider: ethers.Provider): Promise<number | null> {
  try {
    ...
    const url = `https://api.1inch.io/v5.0/137/quote?fromTokenAddress=${tokenAddress}&toTokenAddress=${WETH_ADDRESS}&amount=${amount.toString()}`;
    const { data } = await axios.get(url, { timeout: TIMEOUT });
...
async function fetchFromSushiSwap(address: string): Promise<number | null> {
  try {
    ...
    const { data } = await axios.post(
      "https://api.thegraph.com/subgraphs/name/sushiswap/polygom-exchange",
      { query },
      { timeout: TIMEOUT }
    );
