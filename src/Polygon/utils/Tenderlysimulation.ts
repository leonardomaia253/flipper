  ...
  try {
    ...
    log.info("Starting Tenderly simulation", {
      ...
      options: {
        ...
        networkId: options.networkId || '137', // Default to Avalanche
        save: options.save,
      }
    });
    ...
    const simulationPayload = {
      network_id: options.networkId || '137', // Avalanche
...
export async function estimateBundleProfitability(
  ...
  try {
    log.info("Estimating bundle profitability", {
      ...
      txCount: transactions.length,
      watchAddress,
      tokenAddress: options.tokenAddress || 'MATIC'
    });

    const simulation = await simulateBundleWithTenderly(
      transactions,
      {
        networkId: options.networkId || '137',
        save: false,
