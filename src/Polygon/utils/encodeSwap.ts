...
import {uniswapv3Router,quickswapRouter, quickswapv3Router,sushiswapv2Router} from "../constants/addresses";
...
const recipient = EXECUTOR_CONTRACTPOLYGON


export async function buildQuickswapSwap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(quickswapRouter, UNISWAP_V2_ROUTER_ABI);
    ...
    console.error("Erro ao gerar calldata QuickSwap:", error);
    throw error;
  }
}
...
export async function buildSushiswapV2Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(sushiswapv2Router, UNISWAP_V2_ROUTER_ABI);
...
export async function buildQuickswapV3Swap(swap: DexSwap): Promise<BuiltSwapCall> {
  try {
    const router = new ethers.Contract(quickswapv3Router, UNISWAP_V3_ROUTER_ABI);
