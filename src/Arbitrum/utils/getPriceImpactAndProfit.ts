...
// Função principal
const cacheGetAmountsOut: Map<string, BigNumber[]> = new Map();
...
export async function getPriceImpactAndProfit(
  ...
  try {
    ...
    if (
      ...
      let amountOutMin: BigNumber = BigNumber.from(0);
      let path: string[] = [];
      ...
      let amountsOut: BigNumber[] | undefined = cacheGetAmountsOut.get(key);

      if (!amountsOut) {
        await throttle();
        amountsOut = await routerV2Contract.getAmountsOut(amountIn, path);

        if (!amountsOut || amountsOut.length === 0) return null;
      ...
      const estimatedProfit = expectedAmountOut
        .sub(amountOutMin)
        .sub(gasCost.mul(125).div(100)); // gasCost * 1.25 com precisão
    ...
    if (["exactInputSingle", "exactOutputSingle"].includes(methodName)) {
     ...
     const key = cacheKeyQuoteExactInputSingle(params);
      let amountOutQuoted: BigNumber | undefined = cacheQuoteExactInputSingle.get(key);

       if (!amountOutQuoted) {
      await throttle();
      const quoted = await quoterV3Contract.callStatic.quoteExactInputSingle(
        params.tokenIn,
        params.tokenOut,
        params.fee,
        params.amountIn,
      ...
      amountOutQuoted = quoted;
      cacheQuoteExactInputSingle.set(key, quoted);
    }


      if (!amountOutQuoted) return null; // <- garante que nunca será undefined aqui
  ...
  const estimatedProfit = amountOutQuoted
    .sub(params.amountOutMinimum)
    .sub(gasCost.mul(125).div(100));

  return { priceImpact, estimatedProfit };
}
      if (methodName === "exactOutputSingle") {
        await throttle();
        const amountInQuoted = await quoterV3Contract.callStatic.quoteExactOutputSingle(
          params.tokenIn,
          params.tokenOut,
          params.fee,
          params.amountOut,
        ...
        const estimatedProfit = amountInQuoted
          .sub(params.amountInMaximum)
          .sub(gasCost.mul(125).div(100));
