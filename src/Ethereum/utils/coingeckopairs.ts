...
const COINGECKO_ARBITRUM_URL = 'https://tokens.coingecko.com/ethereum-one/all.json';
const OUTPUT_FILE = path.join(__dirname, 'ethereum_tokens.json');
...
export async function fetchTopTokensEthererum(
  limit: number = 200
): Promise<TokenInfo[]> {
  const now = Date.now();

  if (cache && now - cache.timestamp < CACHE_DURATION_MS) {
    console.log('✅ Usando cache de tokens Ethererum.');
  ...
  currentFetchPromise = (async () => {
    try {
      ...
      console.log('✅ Tokens da Ethererum atualizados e cacheados.');
      ...
      console.warn('⚠️ Usando fallback local com tokens mais comuns da Ethererum.');
